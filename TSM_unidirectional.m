% *** Thermal System Modeling (Unidirectional) + Simple Regression Analysis ***
...
resDir = '/media/SHAYAN_HDD/Results/TSM_unidirectional'; % Directory of Results
...
for j = 1:size(specific_heat,2) % Number of elements in the vectors of thermal properties.
    ...
    for k = 1:size(temp_vect,2)
        ...
        if temp_vect(k) <= internal_temp(j,k)
            
            internal_temp(j,k+1) = internal_temp(j,k); % Keep the internal temperature the same.
