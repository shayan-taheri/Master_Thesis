% *** Performing Generalized Linear Regression for Different Distribution ***
...
clear all;
close all;
distribution = 'poisson'; % Type of Distribution
link_function = 'log'; % Corresponding Link Function
card = 'b5c7';
...
rsq_adj = 1 - (SSresid/SStotal)*(length(y)-1)/(length(y)-length(coef_vals));
% ---- Plotting Results ----
fig_id = figure();
plot(x,y,'*b');
hold on;
...
title(['Generalized Linear Regression Analysis - Distribution: ',num2str(distribution)], 'FontSize', 14);
set(gca, 'fontsize', 12);
...
text(max(x)-0.014,min(y)+0.00007,['R^2 = ',Rsquare,' , ', 'R^2 Adjusted = ',Rsquare_Adj],'FontSize',12,'FontName','Times New Roman');
% ---- Saving Results ----
saveas(fig_id,['/media/SHAYAN_HDD/Results/reg_res/',card,'_',num2str(distribution),'.pdf']);
% -------------------------------