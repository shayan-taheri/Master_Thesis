...
% ---- Norm of Aligned Records ----
y = zeros(1,tRec); % Vector for Matched Filter Output
load([testDir,'/',card,'_',card,'.mat'],'m0');
y = m0.recs_AbsMean;
y = y(g_ix); % Using Mean-Absolute of aligned records that are good.
% ---- Linear Regression ----
p = polyfit(x,y,deg);
yfit = polyval(p,x);
...
ylabel('Mean of Absolute Value of Aligned Records', 'FontSize', 14);
title(['Simple Linear Regression Analysis - Degree ',num2str(deg)], 'FontSize', 14);
set(gca, 'fontsize', 12);
fig_leg = legend ('Obtained Data','Estimated Data');
set(fig_leg,'FontSize',10);
hold off;
% ---- Representing R-Squared ----
Rsquare = num2str(rsq);
Rsquare_Adj = num2str(rsq_adj);
text(0.288,1.958,['R^2 = ',Rsquare,' , ', 'R^2 Adjusted = ',Rsquare_Adj],'FontSize',12,'FontName','Times New Roman');
% ---- Saving Results ----
saveas(fig_id,['/media/SHAYAN_HDD/Results/Collection_2/test_res_mean/figs/',card,'_',num2str(deg),'.pdf']);
% -------------------------------